...
import network_tomography as nt
class multi_armed_bandit:
    ...
    def update_MBA_variabels_train(self, G, x, explored_edge_set):
        edges=list(G.edges)
        for i in range(len(x[0])):
            if x[0][i]!=0:
                edge=edges[i]
                self.logger.debug(f"edge {edge} is computed with the value {x[0][i]}, its realtime delay is {G[edge[0]][edge[1]]['delay']} ")
                self.Dict_edge_theta[edge] = (self.Dict_edge_theta[edge] * self.Dict_edge_m[edge] + x[0][i]) / (
        ...
        for edge in explored_edge_set:
            if edge not in edges:
                edge=(edge[1],edge[0])
            #self.Dict_edge_theta[edge] = (self.Dict_edge_theta[edge] * self.Dict_edge_m[edge] + G[edge[0]][edge[1]]['delay_mean']) / (
            #        self.Dict_edge_m[edge] + 1)
            # to maximize the error give the unidentified edges 0
            self.Dict_edge_theta[edge] = (self.Dict_edge_theta[edge] * self.Dict_edge_m[edge] + 0) / (
                    self.Dict_edge_m[edge] + 1)
            self.Dict_edge_m[edge] = self.Dict_edge_m[edge] + 1
            self.logger.debug(f"updating {edge} with 0")
    ...
    def LLC_policy_exploitation(self, G, monitor1, monitor2):
        ...
        #self.logger.debug(self.Dict_edge_m)
        for edge in G.edges:
            # llc_factor=Dict_edge_theta[edge] + math.sqrt((len(G.edges) + 1) * math.log(t) / Dict_edge_m[edge])
            # print(f"llc_factor: {llc_factor}")
            # G[edge[0]][edge[1]]["llc_factor"] = self.Dict_edge_theta[edge] + math.sqrt(self.Dict_edge_m[edge]/(len(G.edges) + 1) * math.log(self.t)
            #for #node 50, the parameter should be 0.1
            llc_factor= self.Dict_edge_theta[edge] - math.sqrt(
                (len(G.edges) + 1) * math.log(self.t) / self.Dict_edge_m[edge])
            if llc_factor < 0:
                print(f"{edge} lcc factor is negtive")
    ...
    def train_llc(self,G, time, monitor_pair_list,source, destination):
            ...
            for i in range(time-self.t):
                ...
                for monitor_pair in monitor_pair_list:
                    m1=monitor_pair[0]
                    m2=monitor_pair[1]
                    shortest_path=self.LLC_policy_exploitation(G, m1, m2)
                ...
                #with NT
                '''
                b=self.end_to_end_measurement(G,path_list)
                ...
                self.update_MBA_variabels_train(G,x,explored_edge_set)
                '''
            ...
            self.plotter.plot_edge_delay_difference_alongtime(30, 35,self.edge_delay_difference_list)
            ...
            expo_count=0
            print(f"computed_edge_num: {computed_edge_num}")
