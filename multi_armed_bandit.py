...
class multi_armed_bandit:
    ...
    def __init__(self, topo, logger, directory, nt):
        ...
        }  #the observed actual average delay-mean for each edge
        self.Dict_edge_m = {
        }  #the counter for how many times this edge has been observed
        self.t = 1
        self.logger = logger
    ...
    def Initialize(self, G, monitors):
        ...
        delay_difference = []
        for edge in G.edges:
            delay_difference.append(
                abs(self.Dict_edge_theta[edge] -
                    G[edge[0]][edge[1]]['delay_mean']))
        self.edge_delay_difference_list.append(delay_difference)
        plt.savefig(self.directory +
                    'delay difference from mean after initialization',
                    format="PNG",
                    dpi=300,
                    bbox_inches='tight')
        self.plotter.plot_edge_exploitation_times_bar('After initialization',
                                                      self.Dict_edge_m)
        self.edge_exploration_times.append(
            [v for k, v in self.Dict_edge_m.items()])
        ...
        plt.savefig(self.directory +
                    '# of edge exploration after initialization ',
                    format="PNG",
                    dpi=300,
                    bbox_inches='tight')
        plt.close()
    ...
    def find_path(self, G, edge_G, left_node, right_node, source, destination):
        ...
        if (source in G_l.nodes and left_node in G_l.nodes):
            try:
                shortest_path_l = nx.shortest_path(G_l,
                                                   source=source,
                                                   target=left_node,
                                                   weight='weight',
                                                   method='dijkstra')
                #self.logger.debug("shortest path from %s to %s: %s" %(source, left_node,shortest_path_l))
                pathpair_list_l = self.construct_pathPair_from_path(
                    shortest_path_l)
                G_r = G.copy()
                # print(G_r.nodes)
            ...
            if (destination in G_r.nodes and right_node in G_r.nodes):
                try:
                    shortest_path_r = nx.shortest_path(G_r,
                                                       source=right_node,
                                                       target=destination,
                                                       weight='weight',
                                                       method='dijkstra')
                    #.logger.debug("shortest path from %s to %s: %s" %(right_node,destination,shortest_path_r))
                    pathpair_list_r = self.construct_pathPair_from_path(
                        shortest_path_r)
    ...
    def update_MBA_variabels(self, G, pathpair):  ##different
        for e in pathpair:
            if e not in list(G.edges):
                ...
                edge = e
            self.Dict_edge_theta[edge] = (
                self.Dict_edge_theta[edge] * self.Dict_edge_m[edge] +
                G[edge[0]][edge[1]]['delay']) / (self.Dict_edge_m[edge] + 1)
            self.Dict_edge_m[edge] = self.Dict_edge_m[edge] + 1

    def update_MBA_variabels_with_NT(self, G, x, explored_edge_set,
        ...
        for edge in explored_edge_set:
            if edge not in edges:
                ...
                edge_g = edge
            #as a base line approach, the unidentifiable links will be assigned with average delay(total_path_delay/path_length)
            self.Dict_edge_theta[edge_g] = (
                self.Dict_edge_theta[edge_g] * self.Dict_edge_m[edge_g] +
                edge_average_delay_Dict[edge]) / (self.Dict_edge_m[edge_g] + 1)
    ...
    def optimal_path(self, G, monitor_pair_list):
        ...
        for monitor_pair in monitor_pair_list:
            optimal_path = nx.shortest_path(G,
                                            ...
                                            target=monitor_pair[1],
                                            weight='delay_mean',
                                            method='dijkstra')
            #self.logger.info("optimal path: %s" %(optimal_path))
            optimal_delay = 0
            for i in range(len(optimal_path) - 1):
                optimal_delay += G[optimal_path[i]][optimal_path[
                    i + 1]]["delay_mean"]
    ...
    def LLC_policy(self, G, monitor1, monitor2, llc):
        ...
        for edge in G.edges:
            ...
            if llc_factor < 0:
                ...
                print("edge (%s %s) got negtive lcc factor" %
                      (edge[0], edge[1]))
            else:
    ...
    def train_llc(self, G, time, monitor_pair_list, llc_factor):
        optimal_delay_dict, optimal_path_dict = self.optimal_path(
            G, monitor_pair_list)
        optimal_links = []
        ...
        total_mse_array = []
        total_rewards_dict = {
        }  #in the current implementation, it is for only one pair of monitors
        computed_edge_num = []
        ...
        for link in optimal_links:
            optimal_edges_delay_difference_after_inti.append(
                abs(self.Dict_edge_theta[link] -
                    G[link[0]][link[1]]["delay_mean"]))
        ...
        for i in range(time):
            ...
            total_mse = 0
            for edge in G.edges:
                total_mse += (self.Dict_edge_theta[edge] -
                              G[edge[0]][edge[1]]['delay_mean'])**2
            ...
            for monitor_pair in monitor_pair_list:
                ...
                if shortest_path == optimal_path_dict[
                        monitor_pair] and self.t >= time - 1000:
                ...
                total_diff += abs(
                    nx.path_weight(G, optimal_path_dict[monitor_pair], "delay")
                    - nx.path_weight(G, shortest_path, "delay"))
            ...
            diff_of_delay_from_optimal_real_time.append(
                total_diff / len(optimal_delay_dict))
            path_list = []
            ...
            for i in range(len(average_delay_list)):
                for edge in path_list[i]:
                    ...
                    edge_average_delay_list_dict[edge].append(
                        average_delay_list[i])
            for edge in edge_average_delay_list_dict:
                edge_average_delay_dict[edge] = sum(
                    edge_average_delay_list_dict[edge]) / len(
                        edge_average_delay_list_dict[edge])
            ...
            for path in path_list:
                for edge in path:
                    if (edge[0], edge[1]) not in explored_edge_set and (
                            edge[1], edge[0]) not in explored_edge_set:
            ...
            self.update_MBA_variabels_with_NT(G, x, explored_edge_set,
                                              edge_average_delay_dict)
        ...
        average_optimal_path_selected_rate = np.average(
            np.array(rate_optimal_path_selected))
        correct_shortest_path_selected_rate_array = np.array(
            correct_shortest_path_selected_rate)
        average_optimal_path_selected_rate_among_monitor_pairs = np.average(
            np.array(correct_shortest_path_selected_rate_array[-1000:]))
        avg_diff_of_delay_from_optimal = (
            sum(diff_of_delay_from_optimal_real_time) /
            len(diff_of_delay_from_optimal_real_time))
        rewards_mse_list = self.compute_rewards_mse(total_rewards_dict,
                                                    optimal_delay_dict)
        average_regret_list = self.compute_regret(total_rewards_dict,
                                                  optimal_delay_dict)
        ...
        self.plotter.plot_diff_from_optimal_path_of_selected_shortest_paths(
            diff_of_delay_from_optimal_real_time)
        #plot the delay difference from the mean along time
        self.plotter.plot_edge_delay_difference_alongtime(
            0, 15, self.edge_delay_difference_list, '0-15')
        self.plotter.plot_edge_delay_difference_alongtime(
            15, 30, self.edge_delay_difference_list, '15-30')
        self.plotter.plot_edge_delay_difference_alongtime(
            30, 35, self.edge_delay_difference_list, '30-35')

        self.plotter.plot_edge_exploitation_times_bar('t=3000',
                                                      self.Dict_edge_m)
        plt.savefig(self.directory + '# of edge exploration after training',
                    format="PNG",
                    dpi=300,
                    bbox_inches='tight')
        plt.close()
        self.edge_exploration_times.append(
            [v for k, v in self.Dict_edge_m.items()])
        self.plotter.plot_edge_exploitation_times_bar_combined(
            self.edge_exploration_times)
        self.plotter.plot_rate_of_correct_shortest_path(
            correct_shortest_path_selected_rate)  # implement this function
        ...
        self.t = 1
        for link in optimal_links:
            optimal_edges_delay_difference_after_training.append(
                abs(self.Dict_edge_theta[link] -
                    G[link[0]][link[1]]["delay_mean"]))
        self.plotter.plot_edge_delay_difference_for_some_edges(
            optimal_edges_delay_difference_after_inti,
            optimal_edges_delay_difference_after_training)
        average_computed_edge_num = sum(computed_edge_num) / len(
            computed_edge_num)
    ...
    def compute_rewards_mse(self, total_rewards_dict, optimal_delay_dict):
        ...
        for i in range(len(total_rewards_dict[key_list[0]])):
            for key in key_list:
                ...
                '''sulution 2'''
                sum_square += (total_rewards_dict[key][i] -
                               optimal_delay_dict[key])**2
                '''solution 3'''
    ...
    def compute_regret(self, total_rewards_dict, optimal_delay_dict):
        ...
        for i in range(len(total_rewards_dict[key_list[0]])):
            ...
            for key in key_list:
                sum_rewards_Dict[key] += total_rewards_dict[key][i]
                regret = sum_rewards_Dict[key] - (i +
                                                  1) * optimal_delay_dict[key]
    ...
    def plot_edge_delay_difference_at_different_time_point(self, G):
        if self.t == 1000:
            delay_difference1 = []
            for edge in G.edges:
                delay_difference1.append(
                    abs(self.Dict_edge_theta[edge] -
                        G[edge[0]][edge[1]]['delay_mean']))
            self.edge_delay_difference_list.append(delay_difference1)
            self.plotter.plot_edge_delay_difference(G, self.Dict_edge_theta)
            plt.savefig(self.directory +
                        'delay difference from mean at t=1000',
                        format="PNG",
                        dpi=300,
                        bbox_inches='tight')
            plt.close()
        if self.t == 2000:
            delay_difference2 = []
            for edge in G.edges:
                delay_difference2.append(
                    abs(self.Dict_edge_theta[edge] -
                        G[edge[0]][edge[1]]['delay_mean']))
            self.edge_delay_difference_list.append(delay_difference2)
            self.plotter.plot_edge_delay_difference(G, self.Dict_edge_theta)
            plt.savefig(self.directory +
                        'delay difference from mean at t=2000',
                        format="PNG",
                        dpi=300,
                        bbox_inches='tight')
            plt.close()
        if self.t == 3000:
            delay_difference3 = []
            for edge in G.edges:
                delay_difference3.append(
                    abs(self.Dict_edge_theta[edge] -
                        G[edge[0]][edge[1]]['delay_mean']))
            self.edge_delay_difference_list.append(delay_difference3)
            self.plotter.plot_edge_delay_difference(G, self.Dict_edge_theta)
            plt.savefig(self.directory +
                        'delay difference from mean at t=3000',
                        format="PNG",
                        dpi=300,
                        bbox_inches='tight')
            plt.close()
