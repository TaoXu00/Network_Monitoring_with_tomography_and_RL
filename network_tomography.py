...
class network_tomography:
    ...
    def LPSolver(self, path_matrix, b):
        # python main.py Barabasi 20 2 1 1
        # exit(0)
        x = {}
        print('len', len(path_matrix[0]))
        for i in range(len(path_matrix[0])):
            x[i] = pulp.LpVariable(str(i),
                                   lowBound=0,
                                   upBound=sys.maxsize,
                                   cat='Continuous')
        compute_model = pulp.LpProblem(name="task_name")
        for i in range(len(path_matrix)):
            compute_model += (sum([
                path_matrix[i][j] * x[j] for j in range(len(path_matrix[0]))
            ]) == b[0][i])
        compute_model.solve()
        # print('Status:', pulp.LpStatus[compute_model.status])
        x.clear()
        for i in range(len(path_matrix[0])):
            x[i] = 0
        for i in compute_model.variables():
            if i.name == '__dummy':
                continue
            x[int(i.name)] = i.varValue
        x2 = []
        count = 0
        for i in range(len(path_matrix[0])):
            x2.append(x[i])
    ...
    def nt_engine(self, G, path_list, b):
        ...
        x2, count = self.LPSolver(path_matrix, b)
        return x2, count
    ...
    def upper_triangular(self, M):
        ...
        M = np.concatenate(
            (M[np.any(M != 0, axis=1)], M[np.all(M == 0, axis=1)]), axis=0)
    ...
    def back_substitution(self, upper_triangular):
        ...
        upper_triangular_after_deletion = np.delete(upper_triangular, delete,
                                                    0)
        ...
        last_row = upper_triangular_after_deletion[
            len(upper_triangular_after_deletion) - 1]
        ...
        for i in range(total_row - 2, -1, -1):
            ...
            for k in range(pivot + 1, n - 1):
                if upper_triangular_after_deletion[i][k] != 0 and x[k] == 0:
                    ...
                    x[pivot] = x[
                        pivot] - upper_triangular_after_deletion[i][k] * x[k]
